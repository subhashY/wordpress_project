		...
		public function display_element( $element, &$children_elements, $max_depth, $depth=0, $args, &$output ) {
			...
			if ( ! empty( $children_elements[$element->$id_field] ) && ( $depth == 0 )
				...
				if ( true == get_theme_mod( 'ocean_menu_arrow_down', true ) ) {
					$element->title .= ' <span class="nav-arrow fas fa-angle-down"></span>';
				}
			}
			...
			if ( ! empty( $children_elements[$element->$id_field] ) && ( $depth > 0 ) ) {
				...
				if ( true == get_theme_mod( 'ocean_menu_arrow_side', true ) ) {
					if ( is_rtl() ) {
						$element->title .= '<span class="nav-arrow fas fa-angle-left"></span>';
					} else {
						$element->title .= '<span class="nav-arrow fas fa-angle-right"></span>';
					}
				}
			}
